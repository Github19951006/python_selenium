...
# 点击创建
webdriver_object.find_element(By.CLASS_NAME,'btn-green').click()

add_area_elements = webdriver_object.find_element(By.CLASS_NAME,'add-one-area')
input_element = add_area_elements.find_elements(By.CLASS_NAME,'form-control')
input_element[0].send_keys('南京中医院')
input_element[1].send_keys('13554545435')
input_element[2].send_keys('江苏省南京市中央一路566号')
# 获取文本框的信息，存储到列表中
input_value = [input_element[i].get_attribute('value') for i in range(3)]
print(input_value)
webdriver_object.find_element(By.CLASS_NAME,'btn-xs').click()
